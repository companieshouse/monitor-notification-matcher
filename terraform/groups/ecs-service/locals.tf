...
locals {
  stack_name                 = "follow" # this must match the stack name (cluster) the service deploys into
  ...
  healthcheck_path           = "/monitor-notification-matcher/healthcheck" ##healthcheck path for monitor-notification-matcher
  healthcheck_matcher        = "200"
  vpc_name                   = local.stack_secrets["vpc_name"]
  s3_config_bucket           = data.vault_generic_secret.shared_s3.data["config_bucket_name"]
  app_environment_filename   = "monitor-notification-matcher.env"
  use_set_environment_files  = var.use_set_environment_files
  application_subnet_ids     = data.aws_subnets.application.ids
  application_subnet_pattern = local.stack_secrets["application_subnet_pattern"]
  kafka3_broker_address      = "kafka3-common-kafka-1.aws.chdev.org:9092"
  kafka3_notification_match_topic = "cidev-notification-match"
