...
public class ArrayNodeDeserialiser<T> {
    ...
    public ArrayNodeDeserialiser(ObjectMapper mapper, Class<T> type) {
        this.mapper = mapper;
        this.type = type;
    }
    ...
    public List<T> deserialise(ArrayNode arrayNode) {
        try {
            ...
            throw new NonRetryableException("Unable to deserialise array node: [%s]".formatted(arrayNode.toPrettyString()), ex);
        }
    }
}
