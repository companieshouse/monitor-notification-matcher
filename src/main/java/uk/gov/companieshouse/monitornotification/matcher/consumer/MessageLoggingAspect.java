...
public class MessageLoggingAspect {
    ...
    @Before("execution(* uk.gov.companieshouse.monitornotification.matcher.consumer.MonitorFilingConsumer.consume(..))")
    void logBeforeMainConsumer(JoinPoint joinPoint) {
        logMessage(LOG_MESSAGE_RECEIVED, (Message<?>)joinPoint.getArgs()[0]);
    }

    @After("execution(* uk.gov.companieshouse.monitornotification.matcher.consumer.MonitorFilingConsumer.consume(..))")
